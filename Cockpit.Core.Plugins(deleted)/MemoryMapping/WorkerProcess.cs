...
namespace Cockpit.Core.Plugins.MemoryMapping
{
    ...
    public class WorkerProcess<TWorker> : IDisposable where TWorker : IWorker, new()
    {
        ...
        {
                "using System;" +
                "using Cockpit.Core.Plugins.MemoryMapping;" +
                "using System.Linq;" +
        ...
        private static CompilerResults GenerateOrGetAssemblyForType()
        {
            ...
            var res = codeProvider.CompileAssemblyFromSource(new CompilerParameters
            {
                ...
                OutputAssembly = Path.GetTempPath() + "Cockpit.WorkerHost." + typeof(TWorker).Name + "." + Path.GetFileNameWithoutExtension(Path.GetTempFileName()) + ".exe",
